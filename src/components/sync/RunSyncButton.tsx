...
import { triggerGoogleAdsSync } from '@/lib/api-client';
...
export function RunSyncButton({
  ...
  const run = async () => {
    ...
    setResult(null);
    try {
      const r = await triggerGoogleAdsSync({ from, to });
      setResult(r);
    ...
    <div className="flex flex-col items-stretch gap-2 sm:min-w-[260px]">
      <button onClick={run} disabled={pending} className="ice-pill-btn-gold justify-center" type="button">
        {pending ? (
          ...
          <>
            <i className="bi bi-arrow-repeat" aria-hidden="true" /> Run Google Ads sync
          </>
        )}
      </button>
